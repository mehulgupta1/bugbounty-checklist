...
export default function CheckItem({
  ...
  // Clicking the text cycles status — but skip when the user is selecting/copying text.
  const handleTextClick = useCallback(() => {
    if (isEditMode) return;
    const sel = typeof window !== "undefined" && window.getSelection && window.getSelection();
    if (sel && sel.toString().trim().length > 0) return; // user made a selection → don't toggle
    onCycleStatus();
  }, [isEditMode, onCycleStatus]);
        ...
        <div className="check-content">
          <div className="check-text-row">
            <span
              ...
              onClick={!isEditMode ? handleTextClick : undefined}
            >
