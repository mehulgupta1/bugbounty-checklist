...
import {
  ...
  CATEGORIES_VERSION,
  loadCategoriesVersion,
  saveCategoriesVersion,
} from "../utils/storage";
 ...
 */
// Backfill the `group` label onto saved categories from the current defaults
// (so an existing global store gets grouped without losing progress/edits).
function ensureGroups(cats) {
  return (cats || []).map((platform) => {
    const def = DEFAULT_CATEGORIES.find((d) => d.id === platform.id);
    if (!def) return platform;
    const byId = {}, byName = {};
    for (const s of def.sections || []) { byId[s.id] = s.group; byName[s.name] = s.group; }
    return {
      ...platform,
      sections: (platform.sections || []).map((s) =>
        s.group ? s : { ...s, group: byId[s.id] || byName[s.name] }
      ),
    };
  });
}
...
export default function useChecklist(projectId) {
  // Categories are GLOBAL: one shared checklist structure for every project.
  const [categories, setCategories] = useState(() => {
    const stored = loadGlobalCategories();
    // Reseed from the shipped defaults when the checklist version changed.
    const fresh = stored && loadCategoriesVersion() === CATEGORIES_VERSION ? stored : DEFAULT_CATEGORIES;
    return ensureGroups(fresh);
  });
  ...
  // Load PER-PROJECT data when project changes (categories are global, loaded separately)
  useEffect(() => {
    if (!projectId) return;
    ...
    setTimers((data && data.timers) || {});
  }, [projectId]);

  // Persist categories GLOBALLY whenever they change (shared by all projects)
  useEffect(() => {
  ...
  // Ensure activeTab is valid / initialised
  useEffect(() => {
    if (
      categories.length > 0 &&
      (!activeTab || !categories.some((c) => c.id === activeTab))
    ) {
      setActiveTab(categories[0].id);
    }
  }, [categories, activeTab]);

  // Debounced save of PER-PROJECT data (categories excluded — they are global)
  useEffect(() => {
    if (!projectId) return;
    const timer = setTimeout(() => {
      saveProjectData(projectId, { progress, guides, notes, scope, timers });
    }, 300);
    return () => clearTimeout(timer);
  }, [projectId, progress, guides, notes, scope, timers]);
