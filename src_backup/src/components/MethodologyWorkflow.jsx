...
export default function MethodologyWorkflow({ categories }) {
  const [activeTab, setActiveTab] = useState("response-analyzer"); // response-analyzer, decision-engine, intel, asset-board, kill-chain, whitebox
    ...
    <div className="sandbox-container content-area" style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "12px", flexShrink: 0 }}>
        ...
        <div className="dashboard-tabs" style={{ marginBottom: 0, borderBottom: "none", paddingBottom: 0 }}>
          <button 
            className={`dashboard-tab ${activeTab === "response-analyzer" ? "active" : ""}`} 
            onClick={() => setActiveTab("response-analyzer")}
          >
            🔍 Response Analyzer
          </button>
          <button 
          ...
          <button 
            className={`dashboard-tab ${activeTab === "whitebox" ? "active" : ""}`} 
      ...
      {activeTab === "whitebox" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", overflowY: "auto", paddingRight: "8px" }}>
          <div>
            <h3 style={{ margin: "0 0 16px 0", fontSize: "16px" }}>Raw HTTP Request (Blackbox)</h3>
            <p style={{ fontSize: "13px", color: "var(--color-text-muted)", marginBottom: "12px" }}>
              Paste a raw request from Burp Suite. The AI will reverse-engineer the likely backend code handling this request.
            </p>
            <textarea
              className="vault-input"
              style={{ width: "100%", height: "300px", fontFamily: "monospace", fontSize: "13px", padding: "12px", background: "var(--color-bg-secondary)" }}
              value={rawRequest}
              onChange={(e) => setRawRequest(e.target.value)}
            />
            <button className="btn-primary" style={{ width: "100%", marginTop: "16px" }} onClick={generateHypothesis}>
              🔬 Reverse-Engineer Backend Code
            </button>
          </div>

          <div>
            <h3 style={{ margin: "0 0 16px 0", fontSize: "16px" }}>Hypothetical Backend (Whitebox)</h3>
            {hypothesis ? (
              <div style={{ background: "var(--color-bg-secondary)", border: "1px solid var(--color-border)", borderRadius: "8px", overflow: "hidden" }}>
                <div style={{ background: "var(--color-bg)", padding: "8px 12px", borderBottom: "1px solid var(--color-border)", fontSize: "12px", fontWeight: 600, color: "var(--color-text-faint)" }}>
                  Predicted Stack: {hypothesis.language}
                </div>
                <div style={{ padding: "16px" }}>
                  <pre style={{ margin: 0, fontFamily: "monospace", fontSize: "13px", whiteSpace: "pre-wrap", color: "var(--color-green)", lineHeight: "1.5" }}>
                    {hypothesis.code}
                  </pre>
                </div>
                <div style={{ background: "rgba(239, 68, 68, 0.1)", borderTop: "1px solid rgba(239, 68, 68, 0.2)", padding: "16px" }}>
                  <div style={{ fontSize: "13px", fontWeight: 700, color: "var(--color-red)", marginBottom: "8px" }}>💥 Attack Vector Identified:</div>
                  <div style={{ fontSize: "14px", lineHeight: "1.5" }}>{hypothesis.advice}</div>
                </div>
              </div>
            ) : (
              <div className="empty-state" style={{ height: "300px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                Click 'Reverse-Engineer' to generate the hypothetical backend code.
              </div>
            )}
          </div>
        </div>
